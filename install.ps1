...
$cmdlet  = Join-Path $shimDir 'greenroom.ps1'
$shim    = Join-Path $shimDir 'greenroom.cmd'

Copy-Item (Join-Path $src 'greenroom.ps1') -Destination $cmdlet -Force
Ok "command          : $cmdlet"
...
setlocal
pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File "$cmdlet" %*
exit /b %ERRORLEVEL%
