...
Copy-Item (Join-Path $src '*') -Destination $InstallDir -Force
Ok "scripts          : $InstallDir"

# A .ps1 is not directly invokable, so without a shim every command is
# `pwsh -File <long path> attach`. Generate a .cmd one level ABOVE the install
# directory: ~\.local\bin is normally already on the user PATH, the greenroom\
# subfolder is not. Generated rather than shipped so there is one source of truth
# for the path, and so a custom -InstallDir still produces a working shim.
$shimDir = Split-Path $InstallDir -Parent
$shim    = Join-Path $shimDir 'greenroom.cmd'
@"
@echo off
REM Generated by greenroom install.ps1 -- edits will be overwritten.
setlocal
pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File "$InstallDir\greenroom.ps1" %*
exit /b %ERRORLEVEL%
...
if ($onPath) {
    Ok "                   'greenroom' is callable from anywhere"
} else {
    Warn "$shimDir is not on PATH -- 'greenroom' will not resolve as a bare command."
    Warn 'add it to your user PATH, or call the shim by full path.'
}
...
if (Test-Path $userSettings) {
    try {
        ...
        $hostWide = @($us.permissions.additionalDirectories)
