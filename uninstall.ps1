...
if ($RemoveScripts) {
    ...
    if ($others.Count -gt 0) {
        ...
        # The command and shim sit in a directory greenroom does not own -- with a
        # custom -InstallDir that parent may hold unrelated user files. Deleting by
        # name alone would take someone else's greenroom.ps1 with it, so each file
        # must identify itself as ours before it is removed. Overwriting on install
        # is idempotent by design; deleting is not, which is why only this side
        # checks.
        $shimDir = Split-Path $InstallDir -Parent
        $markers = @{
            'greenroom.ps1' = 'Greenroom.Win'                      # the P/Invoke namespace
            'greenroom.cmd' = 'Generated by greenroom install.ps1' # the generated header
        }
        foreach ($f in $markers.Keys) {
            $p = Join-Path $shimDir $f
            if (-not (Test-Path $p)) { continue }
            $content = Get-Content $p -Raw -ErrorAction SilentlyContinue
            if ($content -and $content -match [regex]::Escape($markers[$f])) {
                Remove-Item $p -Force
                Ok "removed: $p"
            } else {
                Warn "left in place, not recognised as greenroom's: $p"
            }
        }
    }
}
